...
<div>
    ...
    @if (config('services.google.client_id'))
        <div class="mb-4">
            <a href="{{ route('google.redirect') }}" class="btn btn-outline w-full gap-2 border-base-300">
    ...
    <form wire:submit="login" class="space-y-4">
        ...
        <div class="flex items-center gap-2">
            <input wire:model="form.remember" id="remember" type="checkbox" name="remember" class="checkbox checkbox-sm" />
            <label for="remember" class="label cursor-pointer text-sm text-base-content">{{ __('Remember me') }}</label>
        </div>
        ...
        <div class="flex items-center justify-end gap-3">
            ...
            <x-button class="btn-primary" type="submit">{{ __('Log in') }}</x-button>
        </div>
    </form>
</div>
