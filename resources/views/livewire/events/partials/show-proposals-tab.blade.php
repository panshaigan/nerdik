...
>
    <h3 class="mb-1 text-lg font-semibold text-sky-100">{{ __('ui.events.pending_proposals') }}</h3>
    <p class="mb-3 text-sm text-sky-100/80">{{ __('ui.events.pending_proposals_help') }}</p>
    <ul class="divide-y divide-sky-300/15">
        @foreach ($pendingProposals as $proposal)
            ...
            @endphp
            <li class="mb-3 space-y-3 rounded-xl border border-sky-300/25 bg-sky-950/35 p-6">
                @php
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                            <a href="{{ route('activities.show', $pa) }}" class="link link-info min-w-0 break-words text-lg font-medium">
                                {{ $pa->name }}
                            </a>
                            <span class="text-sm text-sky-100/70"> · {{ __('ui.common.by') }} {{ $proposal->creator?->displayName() }}</span>
                        </div>
                    </div>
                    <div class="flex flex-wrap justify-end gap-2">
                        @if ($freeSlotsAll->isEmpty())
                            <span class="text-sm text-sky-100/55">{{ __('ui.events.no_free_slots') }}</span>
                        @elseif ($freeSlots->isEmpty())
                            <span class="text-sm text-sky-100/55">{{ __('ui.events.no_compatible_slots') }}</span>
                        @else
                            @php
                            ...
                            <div class="inline-flex max-w-full flex-wrap items-center gap-1" wire:key="proposal-accept-{{ $proposal->id }}">
                                <x-select
                                    ...
                                    placeholder-value=""
                                    class="select-sm max-w-md min-w-[12rem] flex-1 border-sky-300/30 bg-slate-900/70 text-sky-100"
                ...
                <div class="space-y-1.5 min-w-0">
                    @if ($durationLabel !== null || filled($pa->max_participants))
                        <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm">
                            @if ($durationLabel !== null)
                                <span class="inline-flex shrink-0 items-center gap-1.5 text-sky-100/70" title="{{ __('ui.activities.show_duration') }}">
                            ...
                            @if (filled($pa->max_participants))
                                <span class="inline-flex shrink-0 items-center gap-1.5 tabular-nums text-sky-100/60" title="{{ __('ui.activities.max_participants') }}">
                    ...
                    @if ($proposal->proposedSlots->isNotEmpty())
                        <p class="text-sm text-sky-100/80">
                            <span class="font-medium text-sky-100">{{ __('ui.events.preferred_slots') }}:</span>
                            {{ $proposal->proposedSlots->pluck('name')->join(', ') }}
                        </p>
                    @endif
                    @if ($proposal->preferred_start_time)
                        <p class="text-sm text-sky-100/80">
                            <span class="font-medium text-sky-100">{{ __('ui.events.preferred_time') }}:</span>
