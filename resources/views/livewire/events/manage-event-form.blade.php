    ...
    function initManageEventFormScripts() {
        ...
        if (eventForm) {
            eventForm.addEventListener('keydown', (e) => {
                ...
                if (t.hasAttribute('data-event-series-input')) return;
        ...
        const seriesInput = document.querySelector('[data-event-series-input]');
        const seriesPopup = document.querySelector('[data-event-series-popup]');
        const seriesIdInput = document.querySelector('[data-event-series-id]');
        if (seriesInput && seriesPopup && seriesIdInput) {
            const seriesScope = seriesPopup.parentElement;
            const seriesSuggestions = @json($eventSeriesSuggestions ?? []);
            let seriesShown = [];
            let seriesActive = -1;
            ...
            seriesInput.addEventListener('keydown', (e) => {
                ...
                if (e.key === 'ArrowDown') {
                    ...
                    e.preventDefault();
                    if (seriesActive >= 0 && seriesActive < seriesShown.length) {
