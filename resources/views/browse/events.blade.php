<x-app-layout>
    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
            <form id="ui-browse-events-form" method="GET" class="ui-filter-form ui-filter-form-events space-y-4" data-ui="browse-events-form">
                <div class="card border border-base-300 bg-base-100 p-4 shadow-sm" data-ui="browse-events-filters-card">
                    <div class="flex flex-wrap items-end gap-4">
                        ...
                        />
                        @if ($tags->isNotEmpty())
                            <x-form-select id="tag_id" name="tag_id" :label="__('Tag')" class="ui-field ui-field-tag" data-ui="browse-events-tag-select">
                        ...
                        <x-button id="ui-browse-events-submit" type="submit" class="btn-primary ui-action ui-action-search" data-ui="browse-events-search-submit">{{ __('Search') }}</x-button>
                        @if (request()->hasAny(['q', 'tag_id', 'min_lat', 'max_lat', 'min_lng', 'max_lng']))
                            <x-button id="ui-browse-events-clear" :link="route('browse.events')" class="btn-ghost ui-action ui-action-clear" data-ui="browse-events-clear">{{ __('Clear') }}</x-button>
                        @endif
                    </div>
                </div>
