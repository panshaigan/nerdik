...
final class BrandComponentTest extends TestCase
{
    public function test_brand_lockup_renders_logo_and_name_with_scaled_type(): void
    {
        ...
        $this->assertStringContainsString("--brand-logo-height: {$logo['height']}px", $html);
        $this->assertStringContainsString("--brand-wordmark-ratio: {$logo['wordmark_ratio']}", $html);
    ...
    public function test_brand_lockup_nav_wordmark_follows_configured_ratio(): void
    {
        ...
        $this->assertStringContainsString("--brand-wordmark-ratio: {$logo['wordmark_ratio']}", $html);
        $this->assertSame(
            BrandLogoSources::wordmarkFontSizeForHeight($logo['height'], $logo['wordmark_ratio']),
    ...
    public function test_welcome_page_uses_centered_brand_lockup(): void
    {
        ...
        $response->assertSee("--brand-logo-height: {$logo['height']}px", false);
